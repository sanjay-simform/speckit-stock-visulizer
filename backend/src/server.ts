...
export class TickServer {
  ...
  private setupServer(): void {
    this.wss.on("connection", (ws: WebSocket) => {
      ...
      trackers.forEach((tracker) => {
        ...
        const latestValue =
          tracker.history[tracker.history.length - 1] || tracker.currentValue;
        this.sendToClient(ws, {
          type: "TICK",
          payload: {
            id: `init-${tracker.id}`,
  ...
  private handlePauseTracker(
    ws: WebSocket,
    message: PauseTrackerMessage,
  ): void {
    try {
  ...
  private handleResumeTracker(
    ws: WebSocket,
    message: ResumeTrackerMessage,
  ): void {
    try {
      const { trackerId } = message.payload;
  ...
  private handleRemoveTracker(
    ws: WebSocket,
    message: RemoveTrackerMessage,
  ): void {
    try {
      const { trackerId } = message.payload;
...
// Start server
const server = new TickServer(PORT);
server.start();
